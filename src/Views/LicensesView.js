  ...
  const newInput = (e) => {
    if (e.target.value.length > 0) {
      var filtered = data.original.filter(function (el) {
        if (el.companyName) {
          return el.companyName.toLowerCase().indexOf(e.target.value.toLowerCase()) !== -1;
        } else {
          return null;
        }
      });
    ...
    <div>
      {showNewDialog ? (
        <div className="mb-5 mt-3 border border-gray-300 p-3">
          <InputBox onChange={(e) => setNewCompanyname(e.target.value)} headLine="Company name"></InputBox>
          <button
          ...
          <button onClick={() => create()} type="button" className="inline-flex mb-3  mt-3 pr-2 items-center px-2 py-1 border border-transparent shadow-sm text-base font-sm rounded-md text-white bg-blue-500 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
        ...
        <div className="border p-3 mt-3 mb-5">
          <div className="mb-3">
            ...
            <div className="mt-1 relative flex items-center">
              <input onChange={(e) => newInput(e)} type="text" name="search" id="search" className="shadow-sm focus:ring-blue-500 focus:border-blue-500 block w-full pr-12 sm:text-sm border-gray-300 rounded-md" />
          ...
          <Table onSelect={(e) => props.onLicenseSlected(e)} licenses={data.filtered}></Table>
        </div>
      )}
    </div>
  );
